...
import { createCheckItemAction } from '@/features/trips/checkList/api/checkItemCreate.actions';
...
export default function CheckItemCreateForm({ tripId, total }: Props) {
  ...
  const { mutateAsync: createCheckItem } = useMutation({
    mutationFn: createCheckItemAction,
    onSuccess: () => {
